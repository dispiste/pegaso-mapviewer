...
function initSearchTools() {
				...
				store : new Ext.data.JsonStore(
						{
							...
							fields : [
									...
									},
									{
										name : "bounds",
										convert : function(v, rec) {
											var ne = rec.geometry.viewport
													.getNorthEast(), sw = rec.geometry.viewport
													.getSouthWest();
							...
							proxy : new (Ext.extend(Ext.data.DataProxy, {
								doRequest : function(action, rs, params,
										reader, callback, scope, options) {
									...
									geocoder.geocode({
										...
										var readerResult = reader
												.readRecords(results);
										callback.call(scope, readerResult,
												options, !!readerResult);
									});
								}
							}))({
								api : {
	...
	loadLayers = new UAB.csw.CSWSearchField(cswconfig);
